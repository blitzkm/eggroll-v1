...
def load_special_challenge(challenge: str) -> Tuple[List[List[str]], int]:
    ...
    Returns:
        Tuple[List[List[str]], int]: The grid representation of the challenge and the maximum moves allowed.
    """
    WALL = "🧱"
    GRASS = "🟩"
    EGG = "🥚"
    ...
    try:
        ...
        if challenge in levels_data["special_levels"]:
            ...
            level_data = levels_data["special_levels"][challenge]
            grid = level_data["grid"]
            max_moves = level_data.get("max_moves", 5)  # Default to 5 if not specified
        else:
            print("Invalid challenge, loading challenge 1.")
            level_data = levels_data["special_levels"]["1"]
            grid = level_data["grid"]
            max_moves = level_data.get("max_moves", 5)
    except FileNotFoundError:
...
def tilt_grid(grid: List[List[str]], direction: str) -> Union[Tuple[List[List[str]], bool, bool], Tuple[str, bool, bool]]:
    ...
    size_rows = len(grid)
    size_cols = len(grid[0]) if size_rows > 0 else 0
    moved = False
    egg_in_nest = False
    ...
    if direction in ['L', 'R']:
        row_range = range(size_rows)
        col_range = range(size_cols - 1, -1, -1) if direction == 'R' else range(size_cols)
    else:
        row_range = range(size_rows - 1, -1, -1) if direction == 'D' else range(size_rows)
        col_range = range(size_cols)

    row_offset, col_offset = offsets[direction]

    for _ in range(max(size_rows, size_cols)):
        for row in row_range:
            for col in col_range:
                if grid[row][col] == "🥚":
                    ...
                    while (
                        0 <= current_row + row_offset < size_rows
                        and 0 <= current_col + col_offset < size_cols
                    ...
                    destination_cell = grid[current_row + row_offset][current_col + col_offset] if 0 <= current_row + row_offset < size_rows and 0 <= current_col + col_offset < size_cols else None
                    if destination_cell == "🪹":
...
def special_levels(player_name: str):
    """Special levels game loop with unique challenges."""
    def save_and_exit(points: int) -> str:
        save_game = input(f"Do you wish to save your score of {points} to the leaderboard? (Y/N): ").strip().upper()
        if save_game == "Y":
            os.system('clear')
    ...
    print("Choose a special challenge:\n (1) EGGXCELLENT EGGROLLER \n (2) Ma-EGGo sa HEART? \n (3) The Amazing EGGRACE")

    try:
        challenge = int(input().strip())
        if challenge not in range(1, 4):
            print("Invalid challenge choice. Defaulting to challenge 1.")
            challenge = 1
    except ValueError:
        print("Invalid input. Defaulting Challenge 1.")
        challenge = 1
    ...
    while True:
        try:
            grid, remaining_moves = load_special_challenge(str(challenge))
        except KeyError:
            typewriter_effect("Congratulations! You completed all special challenges!")
            return save_and_exit(points)
        ...
        while remaining_moves > 0:
            ...
            if move == "1":
                ...
                if confirm_quit == "1":
                    ...
                    os.system('clear')
                    typewriter_effect("Returning to the current challenge...")
                    time.sleep(1)
                    continue
            ...
            if result == "LOSE":
                points -= 20
                display_grid(grid, moves, remaining_moves, points)
                print("You lose! A special egg landed in the wrong spot!\n")
                while True:
                    choice = input("Restart the challenge (R) or quit to the main menu (Q)? ").strip().upper()
                    if choice == "R":
                        os.system('clear')
                        break
                    elif choice == "Q":
            ...
            all_eggs_in_nests = all(cell != "🥚" for row in grid for cell in row)
            if all_eggs_in_nests:
                points += remaining_moves * 5 * challenge
                display_grid(grid, moves, remaining_moves, points)
                typewriter_effect(f"Congratulations! You completed Challenge {challenge}!\n")
                time.sleep(1)
                break

            if remaining_moves == 0:
                os.system('clear')
                typewriter_effect("Out of moves! Game over.\n")
                time.sleep(1)
                os.system('clear')
                while True:
                    choice = input("Restart the challenge (R) or quit to the main menu (Q)? ").strip().upper()
        ...
        if challenge == 3:
            os.system('clear')
            typewriter_effect("Congratulations! You are amazing in mastering all my special levels!\n")
            time.sleep(1)
            os.system('clear')
            return save_and_exit(points)

        challenge += 1
        typewriter_effect(f"Proceeding to Level {challenge}...\n")
        time.sleep(1)
...
def main_menu():
    ...
    while True:
        ...
        if choice == "1":  # Play
            ...
            typewriter_effect(f"Welcome, {player_name}! Choose your mode:\n")
            print("(1) Normal Mode")
            print("(2) Special Levels")
            mode_choice = input("Select mode: ").strip()

            if mode_choice == "1":
                os.system('clear')
                typewriter_effect("Starting Normal Mode...\n")
                time.sleep(1)
                os.system('clear')
                result = game_loop(player_name)
            elif mode_choice == "2":
                os.system('clear')
                typewriter_effect("Entering Special Levels...\n")
                time.sleep(1)
                os.system('clear')
                result = special_levels(player_name)
            else:
                typewriter_effect("Invalid mode selection. Returning to main menu.\n")
                time.sleep(1)
                continue
